...
def wrapper_identity(name, C):
    ...
    pl.savefig('prob2identity_kernelSVMtrain_'+str(name)+'_with C='+str(C)+'.png')
    ...
    pl.savefig('prob2identity_kernelSVMvalidate_'+str(name)+'_with C='+str(C)+'.png')
    
    
    f = open('errors for identity kernel dataset '+str(name)+' with C = '+str(C)+'.txt', 'w')
    f.write('Train err: ')
